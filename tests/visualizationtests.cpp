...
void testNoteRenderCacheStaticFlags()
{
    ...
    require(style && style->material.body.alphaF() > 0.1
                && style->material.body.alphaF() < 0.3,
            "ghost material must remain visible and quieter than a normal note");
    require(style->material.head.alphaF() > style->material.body.alphaF()
                && style->material.body.alphaF() > style->material.tail.alphaF(),
            "ghosts must preserve the attack, hold and pedal material hierarchy");
}
